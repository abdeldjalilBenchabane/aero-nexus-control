...
const MyFlights = () => {
  ...
  const fetchFlights = async () => {
    try {
      setLoading(true);
      // Use either airline_id or the user's own ID (if they are the airline)
      const airlineId = user?.airline_id || user?.id;
      const data = await flightApi.getByAirline(airlineId!);
      setFlights(data);
            ...
            <Dialog open={isAddFlightOpen} onOpenChange={setIsAddFlightOpen}>
              ...
              <DialogContent className="sm:max-w-[800px]">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Create a new flight for your airline
                  </DialogDescription>
                </DialogHeader>
